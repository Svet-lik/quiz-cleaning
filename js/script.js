document.addEventListener('DOMContentLoaded', function () {
  ...
  const quizPercent = document.getElementById('quiz-position__percent-change');
  ...
  formItems.forEach((formItem, formItemIndex) => {
    ...
    formItem.addEventListener('change', event => {
      ...
      const inputChecked = formItem.querySelectorAll('input:checked');

      if (formItemIndex !== formItems.length - 1) {
        quizPercent.textContent = (formItemIndex + 1) * 25;
        answersObj[`step${formItemIndex}`].answers.length = 0;
        inputChecked.forEach((inputCheck) => {
          answersObj[`step${formItemIndex}`].answers.push(inputCheck.value);
        });
  ...
  const sendForm = () => {
    ...
    form.addEventListener('submit', event => {
      event.preventDefault();
      answersObj.step4.userName = document.getElementById('quiz-name').value;
